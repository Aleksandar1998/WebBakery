...
require_once '../../models/salesStatistics/salesModel.php';
if(isset($_SESSION['admin'])){
    if($_SESSION['admin']==-1){
        ...
        if($tip == 0){
            $objArray=(getEarnings($startDate, $endDate, $db));
            if(!empty($objArray)){
                foreach($objArray as $obj){
                    $dateArray  [] = $obj->Datum;
                    $valueArray [] = $obj->Ukupno;
                }
        ...
        if($tip ==1){
            $objArray=(getSoldAmount($startDate, $endDate, $db));
            if(!empty($objArray)){
                foreach($objArray as $obj){
                    $dateArray  [] = $obj->ime;
                ...
                echo json_encode(array('imena'=>$dateArray,'vrednosti'=>$valueArray));
            }
        }
